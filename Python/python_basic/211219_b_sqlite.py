...
# DB 파일 조회(없으면 새로 생성)
conn = sqlite3.connect('C:/python_workspace/python_basic/resource/database.db')  # 본인 DB 파일 경로
...
'C:/python_workspace/python_basic/resource/database.db'
with conn:
    # Dump 출력(데이터베이스 백업 시 중요)
    with open('C:/python_workspace/python_basic/resource/dump.sql', 'w') as f:
